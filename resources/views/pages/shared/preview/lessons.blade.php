...
@section('sidebar-elements')
    ...
    <nav class="lesson-nav-list">
        @foreach($lessons as $i => $lesson_item)
            ...
            <a class="lesson-nav-item"
               href="{{ route($routePrefix . '.preview.blocks', ['course'=>$course,'chapter'=>$chapter,'lesson'=>$lesson_item]) }}">
    ...
    <div class="sb-lesson-nav">
        <a class="sb-nav-btn"
           href="{{ route($routePrefix . '.preview.chapters', ['course'=>$course]) }}">
            ‹ Chapters
        </a>
    </div>
@endsection
...
@section('navigation')
    <div class="navigation">
        ...
        <span>›</span>
        <a href="{{ route($routePrefix . '.preview.chapters', ['course'=>$course]) }}">{{ $course->title }}</a>
        <span>›</span>
    ...
    <div style="max-width:740px;margin:0 auto;padding:28px 28px 60px;">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
            @foreach($lessons as $i => $lesson)
                ...
                @endphp
                <a href="{{ route($routePrefix . '.preview.blocks', ['course'=>$course,'chapter'=>$chapter,'lesson'=>$lesson]) }}"
        ...
        <div style="display:flex;justify-content:space-between;margin-top:32px;padding-top:20px;border-top:1px solid var(--border);">
            <a href="{{ route($routePrefix . '.preview.chapters', ['course'=>$course]) }}"
            ...
            @if($lessons->first())
                <a href="{{ route($routePrefix . '.preview.blocks', ['course'=>$course,'chapter'=>$chapter,'lesson'=>$lessons->first()]) }}"
