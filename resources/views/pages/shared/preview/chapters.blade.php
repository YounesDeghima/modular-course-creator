...
@section('main')
    <div class="breadcrumb">
        <a href="{{ route($routePrefix . '.preview.courses') }}">Courses</a>
        <span class="sep">›</span>
    ...
    @foreach($chapters as $i => $chapter)
        ...
        <div class="ch-nav-item">
            <a class="ch-nav-row {{ request()->route('chapter') == $chapter->id ? 'active' : '' }}"
               href="{{ route($routePrefix . '.preview.lessons', ['course'=>$course,'chapter'=>$chapter]) }}">
                ...
                @foreach($chapter->lessons as $j => $lesson)
                    ...
                    @if($lesson->status == 'published')
                        ...
                        <a class="ls-nav-item {{ $done ? 'done' : '' }}"
                           href="{{ route($routePrefix . '.preview.blocks',['course'=>$course,'chapter'=>$chapter,'lesson'=>$lesson]) }}">
