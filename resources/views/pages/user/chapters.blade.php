            ...
            <ol class="chapters">
                @foreach($chapters as $chapter)
                    ...
                    <li>
                        ...
                        <a href="{{route('user.preview.lessons',['course'=>$course,'chapter'=>$chapter])}}">{{$chapter->title}}</a>
                            ...
                            @foreach($chapter->lessons as $lesson)
                                @if($lesson->status=='published')
                                    ...
                                    @if($lesson->progressForUser($id) && $lesson->progressForUser($id)->progress > 90)
                                        ...
                                        <li><a style="color: #2ecc71" href="{{route('user.preview.blocks',['course'=>$course,'chapter'=>$chapter,'lesson'=>$lesson])}}">{{$lesson->title}}</a></li>
                                    @else
                                        <li><a href="{{route('user.preview.blocks',['course'=>$course,'chapter'=>$chapter,'lesson'=>$lesson])}}">{{$lesson->title}}</a></li>
                                    @endif
