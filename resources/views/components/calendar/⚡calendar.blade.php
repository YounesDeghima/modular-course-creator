...
new class extends Component {
    ...
    public function events(): array
    {
        ...
        return Event::select('id', 'title', 'description', 'start_date', 'end_date', 'type', 'visibility')
            ->where(function ($q) use ($userId, $userSectionId) {
                $q->where('visibility', 'global')
                    ->orWhere('user_id', $userId)
                    ->orWhere(function ($q) use ($userSectionId) {
                        $q->where('visibility', 'section')
                            ->whereIn('id', function ($sub) use ($userSectionId) {
                                $sub->select('event_id')
                                    ->from('event_section')          // your pivot table name
...
// ─────────────────────────────────────────
    ...
    public function toggleVisibility(string $visibility): void
    {
        if (in_array($visibility, $this->activeVisibilities)) {
            $this->activeVisibilities = array_values(
                array_filter($this->activeVisibilities, fn($v) => $v !== $visibility)
            );
        } else {
            $this->activeVisibilities[] = $visibility;
        }
    }
...
// ─────────────────────────────────────────
    ...
    public function openEdit(int $id): void
    {
        ...
        $this->f_sections = $e->sections->pluck('section_number')->toArray();
        $this->showModal = true;
    }
...
// ─────────────────────────────────────────
    ...
    public function save(): void
    {
        ...
        $this->validate([
            ...
            'f_type' => 'required|in:exam,vacation,project,assignment',
            'f_visibility' => 'required|in:global,personal,section,compagnie,batallion',
        ]);
        ...
        if ($this->f_visibility === 'section') {
            $event->sections()->sync(
                Section::whereIn('section_number', $this->f_sections)->pluck('id')
            );
        } else {
...
@php
    $types  = ['exam','vacation','project','assignment'];
    $visibilityTypes = ['personal','section','compagnie','batallion'];
    $today  = now()->format('Y-m-d');
    ...
    'section'   => 'bar-section',
    'compagnie'    => 'bar-compagnie',
    'batallion' => 'bar-batallion',
    'personal'   => 'bar-personal',
];
    $visibilityBar = [
        'personal'       => 'bar-personal',
    ...
    $visibilityDot = [
        ...
        'section'   => 'dot-section',
        'compagnie'    => 'dot-compagnie',
        'batallion' => 'dot-batallion',
        'personal'   => 'dot-personal',
    ];
            ...
            <div class="cal-sidebar">
                ...
                <div>
                    ...
                    <div style="display:flex;align-items:center;gap:4px;margin-bottom:8px;">
                        <button wire:click="prevPeriod" class="cal-nav-btn"
                                style="width:24px;height:24px;font-size:13px;">‹
                        </button>
                        <button wire:click="nextPeriod" class="cal-nav-btn"
                                style="width:24px;height:24px;font-size:13px;">›
                        </button>
                    </div>
                    <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:1px;margin-bottom:4px;">
                        @foreach(['S','M','T','W','T','F','S'] as $d)
                            <div
                                style="font-size:9px;text-align:center;color:var(--text-faint);font-weight:500;padding:2px 0;">{{ $d }}</div>
                        @endforeach
                        @foreach($this->calendarDays as $day)
                            ...
                            >{{ (int)substr($day['date'],8,2) }}@if($cnt && $day['date']!==$today)
                                    <span
                                        style="position:absolute;bottom:1px;left:50%;transform:translateX(-50%);width:3px;height:3px;border-radius:50%;background:var(--accent);display:block;"></span>
                                @endif</button>
                        @endforeach
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div style="display:flex;flex-direction:column;gap:2px;margin-top:4px;">
                        @foreach($visibilityTypes as $t)
                            <div
                                class="type-filter{{ in_array($t,$activeVisibilities) ? '' : ' off' }}"
                                wire:click="toggleVisibility('{{ $t }}')"
                            >
                                <span class="type-dot {{ $visibilityDot[$t] }}"></span>
                                {{ ucfirst($t) }}
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div style="display:flex;flex-direction:column;gap:6px;margin-top:4px;">
                        ...
                        @forelse($upcoming as $ev)
                            <div class="upcoming-item" wire:click="openEdit({{ $ev['id'] }})" style="cursor:pointer;">
                                <div
                                    class="upcoming-date">{{ \Carbon\Carbon::parse($ev['start_date'])->format('d M Y') }}</div>
        ...
        <div class="cal-page">
            ...
            <div class="cal-header">
                <div class="cal-nav">
                    ...
                    <button wire:click="goToday" class="cal-nav-btn" style="width:auto;padding:0 10px;font-size:12px;">
                        Today
                    </button>
                </div>
                ...
                <div class="month-view" style="display:flex;">
                    ...
                    <div class="days-grid">
                        @foreach($this->calendarDays as $day)
                            ...
                            >
                                <div
                                    class="day-num{{ $isToday ? ' today' : '' }}{{ !$day['cur'] ? ' other-month' : '' }}">
                                ...
                                @endforeach
                                @if(count($evs) > 3)
                                    <div style="font-size:10px;color:var(--text-faint);padding:1px 4px;">
                                        +{{ count($evs)-3 }} more
                                    </div>
                                @endif
                            </div>
            ...
            @if($view === 'week')
                <div class="week-view" style="display:block;">
                    ...
                    <div class="week-grid" style="border-bottom:1px solid var(--border);flex-shrink:0;">
                        ...
                        @foreach($this->weekDays as $wd)
                            ...
                            <div class="week-day-hdr{{ $wIsToday ? ' today-col' : '' }}">
                                <div
                                    style="font-size:10px;text-transform:uppercase;letter-spacing:.04em;">{{ $dow[\Carbon\Carbon::parse($wd)->dayOfWeek] }}</div>
                                <div
                                    style="font-size:18px;font-weight:500;line-height:1.3;">{{ (int)substr($wd,8,2) }}</div>
                            </div>
                        @endforeach
                    </div>
                    {{-- All-day row --}}
                    <div class="week-grid" style="border-bottom:1px solid var(--border);">
                        <div
                            style="font-size:10px;color:var(--text-faint);padding:4px 6px;text-align:right;border-right:1px solid var(--border);">
                            all day
                        </div>
                        @foreach($this->weekDays as $wd)
                            <div style="border-left:1px solid var(--border-mid);padding:3px;min-height:36px;"
                                 wire:click="openNew('{{ $wd }}')">
                        ...
                        <div class="agenda-date-group">
                            ...
                            @foreach($evs as $ev)
                                <div class="agenda-event" wire:click="openEdit({{ $ev['id'] }})">
                                    ...
                                    <span
                                        class="agenda-event-badge {{ $typeChip[$ev['type']] }}">{{ $ev['type'] }}</span>
                                    @if($ev['visibility']==='global')
                                        <span
                                            style="font-size:10px;color:var(--text-faint);margin-left:4px;">global</span>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                    @empty
                        <div style="text-align:center;padding:40px;color:var(--text-faint);font-size:14px;">No upcoming
                            events.
                        </div>
                    @endforelse
                </div>
            ...
            <div class="cal-modal-backdrop open">
                <div class="cal-modal">
                        ...
                        <div class="cal-form-group">
                            ...
                            <div class="visibility-row">
                                <button type="button"
                                        ...
                                        wire:click="$set('f_visibility','global')">🌍 Global (all users)
                                </button>
                                <button type="button"
                                        class="vis-opt{{ $f_visibility==='personal' ? ' selected' : '' }}"
                                        wire:click="$set('f_visibility','personal')">🔒 Personal (only me)
                                </button>
                                <button type="button"
                                        class="vis-opt{{ $f_visibility==='section' ? ' selected' : '' }}"
                                        wire:click="$set('f_visibility','section')">📋 Section
                                </button>
                            </div>

                            {{-- Section number picker (only when section visibility is chosen) --}}
                            @if($f_visibility === 'section')
                    ...
                    <div class="cal-modal-actions">
                        @if($editingId)
                            <button class="cal-btn-delete" wire:click="delete" wire:confirm="Delete this event?">
                                Delete
                            </button>
                        @endif
                        <button class="cal-btn-cancel" wire:click="closeModal">Cancel</button>
                        <button class="cal-btn-submit"
                                wire:click="save">{{ $editingId ? 'Update' : 'Save event' }}</button>
                    </div>
                </div>
            </div>
        @endif
